...
const MyBoardList = () => {
    ...
    useEffect(() => {
        ...
        const getBoardList = async () => {
            ...
            const {data} = await api.get(`/posts/member/list?page_number=${page_number}&page_size=4&member_id=${member_id}`)
        ...
        const getTotalBoard = async () => {
            ...
            const member_id = jwtUtils.getId(token);
            const {data} = await api.get(`/posts/member/count/${member_id}`);
            return data.total;
        }
        ...
        <div className="boardList-wrapper">
            ...
            <div className="boardList-body">
                {boardList.map((item, index) => (
                    <Card key={item.id} membername={item.member.membername} date={moment(item.created).add(9, "hour").format('YYYY-MM-DD')}
